...
client
	proc/debug_variables_new(datum/D in world)
		set category = "Debug"
		set name = "View Variables NEW (beta)"
		//set src in world
		...
		if(istype(D, /atom))
			...
			#ifdef VARSICON
			if (A.icon)
				body += debug_variable_new("icon", new/icon(A.icon, A.icon_state, A.dir), 0)
			#endif
...
//			body += "<option value='byond://?src=\ref[src];emp=\ref[D]'>Trigger EM pulse</option>"
		...
		for (var/V in names)
			body += debug_variable_new(V, D.vars[V], 0, D)
	...
	proc/debug_variable_new(name, value, level, var/datum/DA = null)
		var/html = ""
	...
	//
			...
			if (L.len > 0 && !(name == "underlays" || name == "overlays" || name == "vars" || L.len > 500))
				...
				if(0)   //(L.vars.len > 0)
					...
					for (var/entry in L)
						if(istext(entry))
							html += debug_variable_new(entry, L[entry], level + 1)
						//html += debug_variable_new("[index]", L[index], level + 1)
						else
							html += debug_variable_new(index, L[index], level + 1)
						index++
					html += "</ul>"
...
/client/proc/view_var_Topic(href,href_list,hsrc)
	...
	if( (usr.client == src) && src.holder )
		. = 1	//default return
		if (href_list["Vars"])
			debug_variables_new(locate(href_list["Vars"]))
...
/*		else if (href_list["rename"])
			...
			if(!istype(DAT,/datum) && !istype(DAT,/client))
				...
				return
			modify_variables_new(DAT, href_list["varnameedit"], 1)
			...
			if(!istype(DAT,/datum) && !istype(DAT,/client))
				usr << "Can't edit an item of this type. Type must be /datum or /client, so anything except simple variables."
				return
			modify_variables_new(DAT, href_list["varnamechange"], 0)
...
/*		else if (href_list["makemonkey"])
			...
			var/mob/living/M = locate(href_list["mobToDamage"])
