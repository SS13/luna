...
/mob/proc/changeling_fakedeath()
	...
	spawn(rand(800,2000))
		if(changeling_power(20,1,100,DEAD))
			...
			C.SetWeakened(0)
			C.radiation = 0
			for(var/datum/organ/external/affecting in C.GetOrgans())
				if(!affecting.status)
...
/mob/living/carbon/proc/changeling_panacea()
	...
	set desc = "Cures diseases, disabilities, oxygen deprivation, toxins and radiation."
...
/mob/proc/changeling_DEATHsting()
	...
	if(!T)	return 0
	T << "<span class='danger'>You feel a tiny prick and your chest becomes tight...</span>"
	T.silent = 20
	T.Paralyse(10)
	T.make_jittery(1000)
