...
/datum/artifact_effect/dnaswitch/DoEffectTouch(var/mob/toucher)
	...
	if(ishuman(toucher) && prob(weakness * 100))
		...
		if(prob(weakness * severity * 0.1))
			var/mob/living/carbon/human/H = toucher
			if(prob(30))
				H.dna.mutantrace = null
			else
				H.dna.mutantrace = pickweight(list("lizard" = 2, "plant" = 2, "jelly" = 1, "golem" = 1, "shadow" = 1, "slime" = 1, "tajara" = 2))
		...
		for(var/mob/living/carbon/human/H in range(src.effectrange,holder))
			...
			if(prob(weakness * 100))
				if(prob(30))
					...
					"\green You feel a tingling sensation in your chest.",\
					"\green Your entire body vibrates.")
				if(prob(weakness * severity * 0.01) && H.dna)
					if(prob(30))
						H.dna.mutantrace = null
					else
						H.dna.mutantrace = pickweight(list("lizard" = 2, "plant" = 2, "jelly" = 1, "golem" = 1, "shadow" = 1, "slime" = 1, "tajara" = 2))
		...
		for(var/mob/living/carbon/human/H in range(200, holder))
			...
			if(prob(weakness * 100))
				if(prob(75))
					...
					"\green You feel a tingling sensation in your chest.",\
					"\green Your entire body vibrates.")
			if(prob(weakness * severity * 0.05) && H.dna)
				if(prob(30))
					H.dna.mutantrace = null
				else
					H.dna.mutantrace = pickweight(list("lizard" = 2, "plant" = 2, "jelly" = 1, "golem" = 1, "shadow" = 1, "slime" = 1, "tajara" = 2))
