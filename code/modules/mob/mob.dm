	...
	switch(src.name)
		...
		if("resist")
			...
			if(usr:handcuffed && usr:canmove && (usr.last_special <= world.time))
				...
				usr << "\red You attempt to remove your handcuffs. (This will take around 1 minutesand you need to stand still)"
			...
			if(usr:handcuffed && (usr.last_special <= world.time) && usr:buckled)
				usr.next_move = world.time + 100
				usr.last_special = world.time + 100
				usr << "\red You attempt to unbuckle yourself. (This will take around 30 seconds and you need to stand still)"
				for(var/mob/O in viewers(usr))
