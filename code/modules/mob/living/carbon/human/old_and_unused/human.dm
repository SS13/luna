...
/mob/living/carbon/human/update_clothing()
	...
	if (dna && dna.mutantrace)
		...
		if (gender == MALE)
			...
			g = "_female"
		overlays += image("icon" = 'genetics.dmi', "icon_state" = "[mutantrace][g][!lying ? "_s" : "_l"]")
		if(face_standing)
			del(face_standing)
		if(face_lying)
			del(face_lying)
	...
	if (M.a_intent == "help")
		...
		if (health > 0)
			...
			if(M == src)
				...
				for(var/datum/oegan/external/org in M.organs)
					if(istype(org, /datum/organ/external/wound))
						contuine
					var/status = ""
