	...
	while(Victim && M.health > -70 && stat != 2)
		...
		if(M in view(1, src))
			...
			if(istype(M, /mob/living/carbon))
				Victim.adjustFireLoss(rand(1,8))
		...
		if(M.health <= -70)
			...
			if(!client)
				if(Victim && !rabid && !attacked)
					for(var/obj/item/weapon/grab/G in Victim.grabbed_by)
						if(G.assailant && G.assailant != Victim)
							if(!(G.assailant in Friends))
								Friends.Add(G.assailant) // no idea why i was using the |= operator
