...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/missile_rack/banana_mortar
	...
	action(target)
		if(!action_checks(target)) return
		set_ready_state(0)
		var/obj/item/projectile/B = new projectile(chassis.loc)
		playsound(chassis, fire_sound, 60, 1)
		B.throw_at(target, missile_range, missile_speed)
		projectiles--
		log_message("[launchtext] from [src.name], targeting [target]. HONK!")
		do_after_cooldown()
		return

	get_equip_info()
		return "[..()] \[<a href='?src=\ref[src];mode=0'>B</a>|<a href='?src=\ref[src];mode=1'>M</a>|<a href='?src=\ref[src];mode=2'>P</a>\]"

	Topic(href,href_list)
		..()
		if(href_list["mode"])
			switch(text2num(href_list["mode"]))
				if(0)
					occupant_message("Switched HONK Mortar to Banana Peel. HONK!")
					projectile = /obj/item/weapon/bananapeel
					launchtext = "Bananed"
				if(1)
